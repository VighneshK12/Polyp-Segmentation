...
def test_1(model, path, dataset):
    ...
    for i in range(num1):
        ...
        gt = (gt > 0).astype(np.float32)   # 🔴 any non-zero pixel → 1.0
        image = image.cuda()
...
def train(train_loader, model, optimizer, epoch, test_path):
    ...
    results = []
    if (epoch + 1) % 1 == 0:
        # ---- BKAI evaluation (single dataset) ----
        dataset = 'ValDataset'  # folder name under opt.test_path
        ...
        print(
            f"[Epoch {epoch}] Mean Dice: {mean_dice:.4f} | "
        ...
        if mean_dice > best:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(best_path, f"best_epoch_{epoch}.pth")
            )
