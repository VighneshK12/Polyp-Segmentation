...
def determine_postprocessing(folder_predictions: str,
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as pool:
        ...
        if do_this:
            ...
            print(f'Removing all but the largest foreground region did not improve results_1!')
        ...
        # then evaluate for each class whether this improved results_1. This is no longer possible because we now support
        ...
        if len(labels_or_regions) > 1:
            for label_or_region in labels_or_regions:
                ...
                if do_this:
                    ...
                    print(f'Removing all but the largest component for {label_or_region} did not improve results_1! '
                          f'Dice before: {round(baseline_results["mean"][label_or_region]["Dice"], 5)} '
                          f'after: {round(pp_results["mean"][label_or_region]["Dice"], 5)}')
...
if __name__ == '__main__':
    trained_model_folder = '/home/fabian/results_1/nnUNet_remake/Dataset004_Hippocampus/nnUNetTrainer__nnUNetPlans__3d_fullres'
