...
class nnUNetPredictor(object):
    ...
    # def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...], slice_step: int):
    def _internal_get_sliding_window_slicers(self, image_size: Tuple[int, ...]):
        ...
        if len(self.configuration_manager.patch_size) < len(image_size):
            ...
            # for d in range(0, image_size[0], slice_step):
            # for d in range(0, image_size[0]):
            for d in range(image_size[0]):
                for sx in steps[0]:
                    for sy in steps[1]:
                        ...
                        # slicers.append(
                        #     tuple([slice(None),
                        #            slice(d, min(image_size[0], d+slice_step), None),
                        #            *[slice(si, si + ti) for si, ti in
                        #              zip((sx, sy), self.configuration_manager.patch_size)]]))
        else:
    ...
    def predict_sliding_window_return_logits(self, input_image: torch.Tensor) \
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                # preallocate results_1 and num_predictions
                ...
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    ...
                    # torch.tile()
                    # workon = workon.repeat((1, 32, 1, 1)).permute(1,0,2,3)
                    # workon = workon.permute(1, 0, 2, 3)
...
def predict_entry_point_modelfolder():
    ...
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 ...
                                                 'my_models. This is useful when the nnunet environment variables '
    ...
    parser.add_argument('-m', type=str, required=True,
                        help='Folder in which the trained my_models is. Must have subfolders fold_X for the different '
                             'folds you trained')
    parser.add_argument('-f', nargs='+', type=str, required=False, default=(0, 1, 2, 3, 4),
                        help='Specify the folds of the trained my_models that should be used for prediction. '
                             'Default: (0, 1, 2, 3, 4)')
    ...
    parser.add_argument('-prev_stage_predictions', type=str, required=False, default=None,
                        help='Folder containing the predictions of the previous stage. Required for cascaded my_models.')
