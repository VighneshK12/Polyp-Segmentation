import sys
sys.path.append("/afs/crc.nd.edu/user/y/ypeng4/nnUNet")
# nnunetv2 = "/afs/crc.nd.edu/user/y/ypeng4/nnUNet/nnunetv2"
from nnunetv2.run.seed import *
...
sys.path.insert(0, '/afs/crc.nd.edu/user/y/ypeng4/.conda/envs/python39/lib')
...
from torch.functional import Sequence
...
def get_trainer_from_args(dataset_name_or_id: Union[int, str],
    ...
    signature = inspect.signature(nnunet_trainer.__init__).parameters

    print(signature)

    nnunet_trainer = nnunet_trainer(plans=plans, configuration=configuration, fold=fold,
                                    dataset_json=dataset_json, unpack_dataset=not use_compressed,
                                    device=device, debug=debug, job_id=job_id)
...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training and pretrained_weights_file is not None:
        raise RuntimeError('Cannot both continue a training AND load pretrained weights_IXI. Pretrained weights_IXI can only '
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        if not only_run_validation:
            dataset_id = None if '122' in str(dataset_name_or_id) \
                else dataset_name_or_id
...
def run_training_entry():
    ...
    parser.add_argument('-pretrained_weights', type=str, required=False, default=None,
                        help='[OPTIONAL] path to nnU-Net checkpoint file to be used as pretrained my_models. Will only '
    ...
    parser.add_argument('--job_id', type=str, default='unknown', required=False,
                        help="qsub job id.")
    ...
    run_training(args.dataset_name_or_id, args.configuration, args.fold, args.tr, args.p, args.pretrained_weights,
                 ...
                 device=device, debug=args.debug, job_id=args.job_id)  # 456311
