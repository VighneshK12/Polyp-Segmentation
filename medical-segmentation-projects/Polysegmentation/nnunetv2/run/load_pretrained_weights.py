...
def load_pretrained_weights(network, fname, verbose=False):
    ...
    for key, _ in model_dict.items():
        if all([i not in key for i in skip_strings_in_pretrained]):
            assert key in pretrained_dict, \
                f"Key {key} is missing in the pretrained my_models weights_IXI. The pretrained weights_IXI do not seem to be " \
                f"compatible with your network."
            assert model_dict[key].shape == pretrained_dict[key].shape, \
                f"The shape of the parameters of key {key} is not the same. Pretrained my_models: " \
                f"{pretrained_dict[key].shape}; your network: {model_dict[key]}. The pretrained my_models " \
    ...
    print("################### Loading pretrained weights_IXI from file ", fname, '###################')
    if verbose:
        print("Below is the list of overlapping blocks in pretrained my_models and nnUNet architecture:")
