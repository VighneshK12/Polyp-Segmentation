...
def evaluate(opt, args):
    ...
    if args.verbose is True:
        ...
        datasets = tqdm.tqdm(opt.Eval.datasets, desc='Expr - ' + method, total=len(
            opt.Eval.datasets), position=0, bar_format='{desc:<30}{percentage:3.0f}%|{bar:50}{r_bar}', leave=False)
    else:
...
def evaluate_1(result_path, pred_root_ori, gt_root_ori, verbose=False, debug_dataset_len=5):
    """
    ...
    for dataset in datasets:
        ...
        if verbose is True:
            ...
            samples = enumerate(zip(preds, gts))
        # loky, threading
        ans = Parallel(-1, backend="loky")(delayed(compute_score)(pred_root, gt_root, sample, Thresholds) for i, sample in samples)
...
if __name__ == "__main__":
    ...
    evaluate_1(result_path, pred_root, gt_root, verbose=True)
