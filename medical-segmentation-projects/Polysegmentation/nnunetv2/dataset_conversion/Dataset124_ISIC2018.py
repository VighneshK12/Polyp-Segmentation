...
def load_and_covnert_case(input_image: str, input_seg: str, output_image: str, output_seg: str,
    ...
    assert np.unique(seg)[0] == 0 and (np.unique(seg)[1] == 255 or \
                                       np.unique(seg)[1] == 1)
...
if __name__ == "__main__":
    ...
    orig_img_dir = join(nnUNet_raw, dataset_name, 'imagesTr')
    orit_gt_dir = join(nnUNet_raw, dataset_name, 'labelsTr')

    maybe_mkdir_p(orig_img_dir)
    maybe_mkdir_p(orit_gt_dir)

    train_source = join(nnUNet_raw, dataset_name)
    test_source = join(nnUNet_raw, dataset_name)
        ...
        for v in valid_ids:
            r.append(
                p.starmap_async(
                    ...
                    ((
                         join(train_source, 'all_images_resized', v.replace(".png", "_0000.png")),
                         join(train_source, 'all_gt_resized', v),

                         join(orig_img_dir, v.replace(".png", "_0000.png")),
                         join(orit_gt_dir, v),
                         50
                     ),)
                )
