...
class nnUNetTrainer(object):
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.best_score = {"miou": (-1, -1.0, -1.0), "dsc": (-1, -1.0, -1.0)}

        config = {
            "base_ch": self.base_ch,
            "block": self.block,
            "use_my_unet": self.use_my_unet
        }
        ...
        # use_my_unet = False
        # network_name = "my_unet"
        if not self.use_my_unet:
            self.output_folder_base = join(self.output_folder_base,
                                           f"{self.job_id}_{self.block}_{self.base_ch}")
        else:
            self.output_folder_base = join(self.output_folder_base,
                                           f"{self.job_id}_{self.network_name}_{self.block}_{self.base_ch}")

        self.output_folder = join(self.output_folder_base, f'fold_{fold}')

        self.initial_lr = 5e-3

        print(f"initial_lr: {self.initial_lr}".center(70, "="))
        ...
        # self.log_folder = join("/afs/crc.nd.edu/user/y/ypeng4/data/trained_models",
        #                        self.plans_manager.dataset_name,
        #                        self.__class__.__name__ + '__' + self.plans_manager.plans_name + "__" + configuration,
        #                        f'fold_{fold}')

        self.log_folder = join(self.output_folder, 'logs')
        maybe_mkdir_p(self.log_folder)

        self.log_file = join(self.log_folder, "training_log_%d_%d_%d_%02.0d_%02.0d_%02.0d.txt" %
        ...
        if self.local_rank == 0:
            self.print_to_log_file(f"I am training on {self.hostname}")
    ...
    def initialize_wandb(self, name="Coronary", project="Coronary",
                         dir="wandb", id=None, config=None):
        run = wandb.init(
            project=project,
            dir=dir,
            name=name,
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if self.network_config is None:
                self.network, self.network_config = self.build_network_architecture(self.plans_manager, self.dataset_json,
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        return get_network_from_plans(**network_configs), network_configs
    ...
    def get_plain_dataloaders(self, initial_patch_size: Tuple[int, ...], dim: int):
        ...
        print(f"batch size: {self.batch_size}".center(50, "="))
        if dim == 2:
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        """
        if self.is_ddp:
            if hasattr(self.network.module, "decoder"):
    ...
    def on_train_epoch_start(self):
        ...
        self.print_to_log_file(
            f"Current learning rate: backbone {np.round(self.optimizer.param_groups[0]['lr'], decimals=8)}, "
            f"others {np.round(self.optimizer.param_groups[1]['lr'], decimals=8)}")
        # lrs are the same for all workers so we don't need to gather them in case of DDP training
        self.logger.log('lrs', self.optimizer.param_groups[1]['lr'], self.current_epoch)
        self.logger.log('backbone_lrs', self.optimizer.param_groups[0]['lr'], self.current_epoch)
    ...
    def on_epoch_end(self):
        ...
        if not self.debug:
            wandb.log(data={"loss/train_loss": self.logger.my_fantastic_logging['train_losses'][-1], \
                            "loss/val_loss": self.logger.my_fantastic_logging['val_losses'][-1]},
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ...
        print(f"loading from : {filename_or_checkpoint}".center(70, "="))

        if isinstance(filename_or_checkpoint, str):
            checkpoint = torch.load(filename_or_checkpoint, map_location=self.device)
        # if state dict comes from nn.DataParallel but we use non-parallel my_models here then the state dict keys do not
        ...
        self.logger.my_fantastic_logging['backbone_lrs'] = \
            self.logger.my_fantastic_logging.get('backbone_lrs',
                                                 self.logger.my_fantastic_logging['lrs'])

        if isinstance(self.logger.my_fantastic_logging['lrs'][0], (list, tuple)):
            self.logger.my_fantastic_logging['lrs'] = [x[1] for x in self.logger.my_fantastic_logging['lrs']]
        if isinstance(self.logger.my_fantastic_logging['lrs'][0], (list, tuple)):
            self.logger.my_fantastic_logging['backbone_lrs'] = [x[0] for x in self.logger.my_fantastic_logging['backbone_lrs']]

        self._best_ema = checkpoint['_best_ema']
        self.inference_allowed_mirroring_axes = checkpoint[
            'inference_allowed_mirroring_axes'] if 'inference_allowed_mirroring_axes' in checkpoint.keys() else self.inference_allowed_mirroring_axes

        self.id = checkpoint.get('wandb_id', None)
        self.network_config = checkpoint.get('network_config', None)

        self.initial_lr = checkpoint.get('initial_lr', self.initial_lr)
        self.backbone_lr = checkpoint.get('backbone_lr', self.backbone_lr)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for k in dataset_val.keys():
                ...
                # for debug purposes
                # export_prediction(prediction_for_export, properties, self.configuration,
                # self.plans, self.dataset_json,
                ...
                if next_stages is not None:
                    for n in next_stages:
                        ...
                        # resample_and_save(prediction, target_shape, output_file, self.plans_manager,
    ...
    def run_training(self, dataset_id=None):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            self.print_to_log_file(f"finish training")
