...
class nnUNetDataLoader2D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        for j, current_key in enumerate(selected_keys):
            # oversampling foreground will improve stability of my_models training, especially if many patches are empty
            # (Lung for example)
            force_fg = self.get_do_oversample(j)
            data, seg, properties = self._data.load_case(current_key)
            # print(type(data), type(seg))
            # print('init', np.unique(seg))
            ...
            data = data[:, selected_slice].astype(np.float)
            seg = seg[:, selected_slice].astype(np.float)
            # print('init 1: ', np.unique(seg))
