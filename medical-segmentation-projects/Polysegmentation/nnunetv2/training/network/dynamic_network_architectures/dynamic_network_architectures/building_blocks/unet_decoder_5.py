...
class UNetDecoder(nn.Module):
    def __init__(self,
        ...
        # cross_conv out: 512, 512, 256, 128, 64, 32

        self.cross_conv = [nn.Sequential(
            nn.Conv2d(total_channels,
            ...
            for s in range(len(encoder.output_channels[:-self.start_stage]))]
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            if s == len(self.stages)-1:
                ...
                for y in skips[1:-1]:
                    if y.shape[-1] < x.shape[-1]:  # up sample
                        dispatch.append(F.interpolate(y, x.shape[-2:]))
                    elif y.shape[-1] > x.shape[-1]:  # down_sample
