...
# from timm.my_models.layers import trunc_normal_, DropPath
import pdb
...
class MBConv(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, expansion=4, kernel_size=3, stride=1, ratio=4, p=0, se=True,
                 norm=nn.BatchNorm2d,
                 act=nn.ReLU):
        super().__init__()
        ...
        self.expand_proj = nn.Identity() if (expansion == 1) else \
            ConvNormAct(in_ch, expanded, kernel_size=1, norm=norm,
                        act=act, preact=True)

        self.depthwise = ConvNormAct(expanded, expanded, kernel_size=kernel_size,
        ...
        self.pointwise = ConvNormAct(expanded, out_ch, kernel_size=1, padding=0,
                                     norm=norm, act=False, preact=True)
        ...
        if in_ch != out_ch or stride != 1:
            self.shortcut = nn.Sequential(
                ConvNormAct(in_ch, out_ch, kernel_size, stride=stride,
                            padding=padding, norm=False, act=False))
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(self, dim, out_ch, stride=1, kernel_size=7, norm=None, act=None, preact=None, drop_path=0.,
        ...
        # self.pwconv2 = nn.Linear(4 * dim, out_ch)
        ...
        if dim != out_ch:
            self.down_sample = nn.Conv2d(dim, out_ch, 3, 1, 1)
    def forward(self, x):
        input = x
        x = self.dwconv(x)
        ...
        if self.down_sample is None:
            ...
            x = self.down_sample(input) + self.down_sample(x)
...
if __name__ == '__main__':
    ...
    conv1 = ConvNormAct(in_ch=3, out_ch=32, kernel_size=3, stride=1, padding=1,
                        groups=1, dilation=1, bias=False, norm=nn.BatchNorm2d,
                        act=nn.ReLU, preact=False)
    # single conv norm relu, exactly the same as ConvNormAct
    conv2 = SingleConv(in_ch=3, out_ch=32, stride=1, norm=nn.BatchNorm2d,
                       act=nn.ReLU, preact=False)

    conv3 = BasicBlock(in_ch=3, out_ch=32, stride=1, norm=nn.BatchNorm2d,
                       act=nn.ReLU, preact=True)   # 2 3*3 conv, with one shortpath: 3*3 conv

    conv4 = Bottleneck(in_ch=3, out_ch=32, stride=1, groups=1, dilation=1,
                       norm=nn.BatchNorm2d, act=nn.ReLU,
                       preact=True)  # 3 3*3 conv, with one shortpath: 3*3 conv

    conv5 = DepthwiseSeparableConv(in_ch=3, out_ch=32, stride=1, kernel_size=3,
                                   padding=1, bias=False)  # 1 conv2 divided into group conv and 1*1 conv

    # avg pool to 1*1, then down the channel, then up the channel, and output 1*1 feature map, finally, multiply the 1*1 feature map with original feature map
    conv6 = SEBlock(in_ch=3, ratio=4, act=nn.ReLU)

    conv7 = MBConv(in_ch=3, out_ch=32, expansion=4, kernel_size=3, stride=1,
                   ratio=4, p=0, se=True, norm=nn.BatchNorm2d,
                   act=nn.ReLU)  # 3*3 conv (pre_act) + depthwise + SE + pointwise, then a shortcut

    conv8 = FusedMBConv(in_ch=3, out_ch=32, expansion=4, kernel_size=3,
                        stride=1, ratio=4, p=0, se=True,
                        norm=nn.BatchNorm2d,
                        act=nn.ReLU)

    conv9 = ConvNeXtBlock(dim=3, out_ch=32, stride=1, kernel_size=7,
                          norm=None, act=None, preact=None, drop_path=0.,
                          layer_scale_init_value=1e-6)

    convs = [conv7, conv8, conv9]
    ...
    """
    os.makedirs("netron", exist_ok=True)
    for conv in convs:
        # script_model = torch.jit.trace(conv, img)
        # script_model.save(f"netron/{conv.__class__.__name__}.pt")
