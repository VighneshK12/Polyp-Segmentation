...
class PVTNetwork_3(nn.Module):
    def __init__(self, channel=9, n_classes=1, dap_k=3, deep_supervision=True):
        super().__init__()
        self.deep_supervision = deep_supervision

        print(f"use Conv2d(7, 1) Conv2d(1, 7) and My attention layer, and"
              f"replace SegConv to PixelShuffle".center(50, "="))
        ...
        self.deconv2 = nn.ConvTranspose2d(channel, n_classes * dap_k ** 2, kernel_size=4, stride=2, padding=1,
                                          bias=False)
        self.deconv3 = nn.ConvTranspose2d(channel, n_classes * dap_k ** 2, kernel_size=4, stride=2,
                                          padding=1, bias=False)
        self.deconv4 = nn.ConvTranspose2d(channel, n_classes * dap_k ** 2, kernel_size=4, stride=2,
                                          padding=1, bias=False)
        self.deconv5 = nn.ConvTranspose2d(channel, n_classes * dap_k ** 2, kernel_size=4, stride=2,
                                          padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        # print(nn.AvgPool2d((3, 3))(nn.PixelShuffle(3)(y)).shape)
        # print(self.seg_outs[0])
...
if __name__ == "__main__":
    model = PVTNetwork_3(n_classes=1)
