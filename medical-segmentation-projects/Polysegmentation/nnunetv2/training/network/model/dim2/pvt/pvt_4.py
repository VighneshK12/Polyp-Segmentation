...
class AttentionLayer_1(nn.Module):
    def __init__(self):
        super().__init__()
        ...
        self.proj = nn.ModuleList([nn.Linear(32, 32) for _ in range(4)])

        # self.mlp = nn.ModuleList([nn.Identity() for _ in range(4)])
        # self.k1 = nn.Linear(32, 16)
        # self.v1 = nn.Linear(32, 16)
        #
        # self.k2 = nn.Linear(32, 16)
        # self.v2 = nn.Linear(32, 16)
        #
        # self.k3 = nn.Linear(32, 16)
        # self.v3 = nn.Linear(32, 16)

        self.q = nn.Linear(32, 32)

        self.conv = nn.Conv2d(32, 32, 1, 1)

        self.norm = nn.ModuleList([nn.LayerNorm(32) for _ in range(4)])
    ...
    def forward(self, xs, anchor):
        ...
        anchor = anchor.flatten(2).transpose(1, 2)
        anchor = self.q(anchor)
        ...
        for i, x in enumerate(xs):
            ...
            res = self.norm[i](self.proj[i](res))
            # res = res + anchor

            # res = res + self.mlp(res)

            res = res.transpose(1, 2).reshape((N, C, H, W))
            ans = ans * res

        # ans = torch.cat(ans, dim=1)
        ans = self.conv(ans) + anchor_copy
        return ans
...
class PVTNetwork_4(nn.Module):
    ...
    def __init__(self, channel=32, n_classes=1, deep_supervision=True):
        ...
        self.seg_outs = nn.ModuleList([
            nn.Conv2d(32, n_classes, 1, 1) for _ in range(4)])
        ...
        self.deconv4 = nn.ConvTranspose2d(32, 32, kernel_size=4, stride=2,
                                          padding=1, bias=False)
        self.deconv4_conv = BasicConv2d(channel * 2, channel, 3, 1, 1)

        # self.deconv5 = nn.ConvTranspose2d(32, 32, kernel_size=4, stride=2,
        #                                   padding=1, bias=False)
        # self.deconv5_conv = BasicConv2d(channel * 2, channel, 3, 1, 1)
...
if __name__ == "__main__":
    model = PVTNetwork_4(n_classes=2, deep_supervision=True)
