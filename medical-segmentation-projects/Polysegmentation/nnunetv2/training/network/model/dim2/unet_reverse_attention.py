...
#
#     def forward(self, x):
#         return 1-F.sigmoid(self.conv(x))

class UNet(nn.Module):
    def __init__(self, in_ch, num_classes, base_ch=16,
                 block='SingleConv', pool=True, deep_supervision=True,
                 use_my_net=False, reverse_setting=0):
        super().__init__()
        self.use_my_net = use_my_net
        self.reverse_setting = reverse_setting
        ...
        # self.ra_map_7 = ReverseAttention(32*base_ch)
        # self.ra_map_6 = ReverseAttention(32*base_ch)
        # self.ra_map_5 = ReverseAttention(16*base_ch)
        # self.ra_map_4 = ReverseAttention(8*base_ch)
        # self.ra_map_3 = ReverseAttention(4 * base_ch)
        # self.ra_map_2 = ReverseAttention(2 * base_ch)
    ...
    def forward(self, x):
        ...
        # reverse_attention_map_7 = self.ra_map_7(x7)  # (1, H/64, W/64)
        ...
        if self.use_my_net:
            ...
            # old
            out = self.up0_0(x7, x6)  # (H/32, W/32)
            # out = out * F.interpolate(reverse_attention_map_7, scale_factor=2, mode='bilinear')

            # out = self.up0_0(x7 * reverse_attention_map_7, x6)  # (H/32, W/32)

        x = self.seg_layers[0](out)
        seg_out.append(x)
        # seg_out.append(self.seg_layers[0](out))

        # reverse_attention_map_6 = self.ra_map_6(out)
        ...
        if self.use_my_net:
            ...
            # out = out * F.interpolate(reverse_attention_map_6, scale_factor=2, mode='bilinear')
            # print(x[:, :1].shape)
            out = self.up0(out,  # * F.softmax(x, dim=1)[:, :1]
                           F.interpolate(F.softmax(x, dim=1)[:, 1:], scale_factor=2, mode='bilinear') * x5)  # 1st out, (H/16, W/16)

        x = self.seg_layers[1](out)
        seg_out.append(x)
        # seg_out.append(self.seg_layers[1](out))

        # reverse_attention_map_5 = self.ra_map_5(out)
        if self.use_my_net:
            out = self.up1(out, self.trans_conv_3(trans_conv(all, x4.shape[-1])))
        else:
            # old
            # out = self.up1(out, x4)  # 2nd out, (H/8, W/8)
            # out = out * F.interpolate(reverse_attention_map_5, scale_factor=2, mode='bilinear')

            # out = self.up1(out * reverse_attention_map_5, x4)  # 2nd out, (H/8, W/8)
            out = self.up1(out,  # * F.softmax(x, dim=1)[:, :1],
                           F.interpolate(F.softmax(x, dim=1)[:, 1:], scale_factor=2, mode='bilinear') *x4)  # 2nd out, (H/8, W/8)

        x = self.seg_layers[2](out)
        seg_out.append(x)
        # seg_out.append(self.seg_layers[2](out))

        # reverse_attention_map_4 = self.ra_map_4(out)
        if self.use_my_net:
            out = self.up2(out, self.trans_conv_4(trans_conv(all, x3.shape[-1])))
        else:
            # old
            # out = self.up2(out, x3)  # 3rd out,  (H/4, W/4)
            # out = out * F.interpolate(reverse_attention_map_4, scale_factor=2, mode='bilinear')

            # out = self.up2(out * reverse_attention_map_4, x3)  # 3rd out,  (H/4, W/4)
            out = self.up2(out,  # * F.softmax(x, dim=1)[:, :1],
                           F.interpolate(F.softmax(x, dim=1)[:, 1:], scale_factor=2, mode='bilinear') * x3)  # 3rd out,  (H/4, W/4)
            # out = out * F.interpolate(reverse_attention_map_4, scale_factor=2, mode='bilinear')
        x = self.seg_layers[3](out)
        seg_out.append(x)
        # seg_out.append(self.seg_layers[3](out))

        # a = trans_conv(all, x2.shape[-1])
        # reverse_attention_map_3 = self.ra_map_3(out)
        if self.use_my_net:
            out = self.up3(out, self.trans_conv_5(trans_conv(all, x2.shape[-1])))
        else:
            # old
            # out = self.up3(out, x2)  # 4th out, (H/2, W/ 2)
            # out = out * F.interpolate(reverse_attention_map_3, scale_factor=2, mode='bilinear')

            # out = self.up3(out * reverse_attention_map_3, x2)  # 4th out, (H/2, W/ 2)
            out = self.up3(out,  #  * F.softmax(x, dim=1)[:, :1],
                           F.interpolate(F.softmax(x, dim=1)[:, 1:], scale_factor=2, mode='bilinear') * x2)  # 4th out, (H/2, W/ 2)

        x = self.seg_layers[4](out)
        seg_out.append(x)
        # seg_out.append(self.seg_layers[4](out))

        # reverse_attention_map_2 = self.ra_map_2(out)
        if self.use_my_net:
            out = self.up4(out, self.trans_conv_6(trans_conv(all, x1.shape[-1])))
        else:
            # old
            # out = self.up4(out, x1)  # 5-th out  (C, H, W)
            # out = out * F.interpolate(reverse_attention_map_2, scale_factor=2, mode='bilinear')

            # out = self.up4(out * reverse_attention_map_2, x1)  # 5-th out  (C, H, W)
            out = self.up4(out,  # * F.softmax(x, dim=1)[:, :1],
                           F.interpolate(F.softmax(x, dim=1)[:, 1:], scale_factor=2, mode='bilinear') * x1)  # 5-th out  (C, H, W)
