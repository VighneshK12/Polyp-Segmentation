...
class UNet16(nn.Module):
    def __init__(self, num_classes=1, num_filters=32, pretrained=False,
        ...
        seg_layers = []
        for i in [512, 256, 256, 256, 64]:
            seg_layers.append(nn.Conv2d(i,
                                        num_classes, 1, 1, 0,
                                        bias=True))
    ...
    def forward(self, x):
        ...
        feature = self.pool(conv5)

        seg_out = []
        seg_out.append(self.seg_layers[0](feature))

        center = self.center(feature)
        seg_out.append(self.seg_layers[1](center))

        dec5 = self.dec5(torch.cat([center, conv5], 1))
        seg_out.append(self.seg_layers[2](dec5))

        dec4 = self.dec4(torch.cat([dec5, conv4], 1))
        seg_out.append(self.seg_layers[3](dec4))

        dec3 = self.dec3(torch.cat([dec4, conv3], 1))
        seg_out.append(self.seg_layers[4](dec3))

        dec2 = self.dec2(torch.cat([dec3, conv2], 1))
        # seg_out.append(self.seg_layers[4](dec2))

        dec1 = self.dec1(torch.cat([dec2, conv1], 1))
        x_out = self.final(dec1)
        seg_out.append(x_out)
