...
class Mlp(nn.Module):
    """ MLP as used in MetaFormer my_models, eg Transformer, MLP-Mixer, PoolFormer, MetaFormer baslines and related networks.
...
class MetaFormer(nn.Module):
    ...
    def forward_features(self, x):
        for i in range(self.num_stage):
            x = self.downsample_layers[i](x)  # (B, C, H, W) - > (B, H, W, C)
            x = self.stages[i](x)
